...
function _getMenu(menu: MenuName, withRoutes = false as boolean | RouteName[], filter = [] as RouteName[]): string[] | Route[] {
  ...
  return (_menus[menu] || []).reduce((a, c) => {
    ...
    if (!route.visible || (Array.isArray(route.visible) && !_checkMenuVisibility(menu, route.visible)))
      return a;
    return [...a, route];
  }, [] as Route[]);
}
...
function _getMenus(menus: MenuName | MenuName[], withRoutes = false as boolean | RouteName[], filter = [] as RouteName[]): string[] | Route[] {
