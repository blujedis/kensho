...
export const stlyeNames = (...styles: (DataType | DataType[])[]) => {
  ...
  return flattened.reduce((a, c) => {
    ...
    a = [...a, c];
    return a;
  }, [] as any[]).filter(v => v !== null && typeof v !== 'undefined' && v !== '').join('; ');
};
...
export function gt(value: string | number, compare: string | number) {
  ...
  else if (typeof value === 'number' && typeof compare === 'number')
    return value > compare;
  return false;
}
...
export function incl(value: string | (DataType | Record<string, unknown>)[], compare: DataType | Record<string, unknown> | (Record<string, unknown> | DataType)[]) {
  if (Array.isArray(compare) && !Array.isArray(value))
    return false; // value cannot include an array.
  if (Array.isArray(value))
    return value.some(v => (compare as any[]).includes(v));
  if (typeof value === 'string' && typeof compare === 'string')
...
export function like(value: unknown, compare: unknown) {
  ...
  if (Array.isArray(value) && Array.isArray(compare))
    return value.some(v => v.includes(compare))
  return false;
}
