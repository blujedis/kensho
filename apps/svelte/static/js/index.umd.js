...
(function (global, factory) {
	...
	function getFirstDefined(obj, props, altProp) {
		...
		if (typeof found !== 'undefined' && typeof altPropVal !== 'undefined') found = altPropVal;
		return found;
	}
	...
	function castDataType(dataType, dataTypeOptions, value) {
		...
		if (isArray) return ensureArray(value).map((v) => COERCE_MAP[cleanType](v, ...dataTypeOptions));
	...
	function createController(options) {
		...
		async function validate(namesOrValues) {
			...
			let names =
				typeof namesOrValues === 'string' || Array.isArray(namesOrValues)
					...
					: null;
			let values =
				!Array.isArray(namesOrValues) && namesOrValues !== null && typeof namesOrValues === 'object'
					? namesOrValues
					: null;
			...
			const invalid =
				(typeof result !== 'undefined' && result !== null) || !!Object.keys(result).length;
			if (!invalid) {
		...
		function getValue(name, cast = options.onCoerce) {
			...
			if (cast) {
				if (cast === true) val = castDataType(conf.dataType, conf.dataTypeOptions, val);
		...
		function getValues(canTransform = true) {
			...
			if (options.mergeUnbound) obj = mergeObject({ ...options.initialValues }, obj);
			if (typeof options.onTransform === 'function' && canTransform) obj = options.onTransform(obj);
			return obj;
		}
		/**
		...
		function bind(...collection) {
			for (const el of collection) {
				...
				const isUnbound =
					el.hasAttribute && el.hasAttribute(options.unboundAttribute || 'data-unbound');
	...
	exports.getProperty = lodash_get;
	exports.hasProperty = lodash_has;
	exports.mergeObject = mergeObject;
	exports.setProperty = lodash_set;
	exports.unflattenMap = unflattenMap;
