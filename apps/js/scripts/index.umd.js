(function (global, factory) {
  ...
  function hashSet$2(key, value) {
    var data = this.__data__;
    data[key] =
      nativeCreate$2 && value === undefined ? HASH_UNDEFINED$2 : value;
    return this;
  }
  ...
  function baseIsNative$2(value) {
    ...
    var pattern =
      isFunction$2(value) || isHostObject$2(value)
        ? reIsNative$2
        : reIsHostCtor$2;
  ...
  function isKey$2(value, object) {
    ...
    return (
      reIsPlainProp$2.test(value) ||
      !reIsDeepProp$2.test(value) ||
  ...
  var stringToPath$2 = memoize$2(function (string) {
    ...
    string.replace(rePropName$2, function (match, number, quote, string) {
      result.push(
        quote ? string.replace(reEscapeChar$2, "$1") : number || match
      );
    });
    return result;
  });
  ...
  function hashSet$1(key, value) {
    var data = this.__data__;
    data[key] =
      nativeCreate$1 && value === undefined ? HASH_UNDEFINED$1 : value;
    return this;
  }
  ...
  function baseIsNative$1(value) {
    ...
    var pattern =
      isFunction$1(value) || isHostObject$1(value)
        ? reIsNative$1
        : reIsHostCtor$1;
  ...
  var stringToPath$1 = memoize$1(function (string) {
    ...
    string.replace(rePropName$1, function (match, number, quote, string) {
      result.push(
        quote ? string.replace(reEscapeChar$1, "$1") : number || match
      );
    });
    return result;
  });
  ...
  function getFirstDefined(obj, props, altProp) {
    ...
    const altPropVal =
      typeof altProp !== "undefined" ? obj[altProp] : undefined;
  ...
  function parseNativeAttributes(el) {
    ...
    attrs.minlength =
      attrs.minlength !== null ? parseInt(attrs.minlength + "") : null;
    attrs.maxlength =
      attrs.maxlength !== null ? parseInt(attrs.maxlength + "") : null;
  ...
  function setInputValue(conf, value) {
    const el = conf.el;
    if (conf.virtual || !el) conf.virtualValue = value;
    else el.value = value + "";
  }
  /**
  ...
  function setCheckboxValue(conf, value) {
    ...
    if (conf.virtual || !el) {
      ...
      if (/(true|1|yes)/.test(value + "")) el.setAttribute("checked", "true");
    }
  }
  /**
  ...
  function setRadioValue(conf, value) {
    ...
    if (conf.virtual || !el) {
      ...
      el.forEach((e) => {
        if (e.value === value) e.setAttribute("checked", "true");
      });
    }
  }
  /**
  ...
  function setSelectValue(conf, value) {
    ...
    function recurseChildren(values, children) {
      for (const c of children) {
        if (c instanceof HTMLOptGroupElement) {
          ...
          const val = c.value;
          if (values.includes(val)) c.selected = true;
    ...
    if (conf.virtual || !el) {
      ...
      const _values = ensureArray(value).filter(
  ...
  function createController(options) {
    ...
    function subscribe(fn) {
      if (!_subscribers.includes(fn)) _subscribers.push(fn);
      return () => {
    ...
    function unsubscribe(fn) {
      if (!fn) _subscribers = [];
      else _subscribers = _subscribers.filter((v) => v !== fn);
    }
    /**
      ...
      return (
        ...
        (() => {
          ...
          if (
            ...
            !_formState.initialized &&
            !hasWarned
          ) {
            console.warn(
              'Failed to validate, validation handler NOT defined. To suppress this warning set "validator:false"'
            );
            if (hasWindow) window.__novalidator__ = 1;
          }
        })
      );
    }
    async function validate(namesOrValues) {
      if (_formState.validating) return _errorState;
      let names =
        typeof namesOrValues === "string" || Array.isArray(namesOrValues)
          ? typeof namesOrValues === "string"
            ? [namesOrValues]
            : namesOrValues
          : null;
      let values =
        !Array.isArray(namesOrValues) &&
        namesOrValues !== null &&
      ...
      if (!invalid) {
        ...
        _errorState = names.reduce((a, c) => {
          ...
          if (options.flattenErrors && errors) a[c] = errors;
          else if (!options.flattenErrors && errors) lodash_set(a, c, errors);
          return a;
        }, {});
      }
    ...
    function getElements() {
      return Object.keys(_elements).reduce(
        (a, c) => (Array.isArray(c) ? [...a, ...c] : [...a, c]),
        []
      );
    }
    /**
    ...
    function getValue(name, cast = options.onCoerce) {
      ...
      val =
        typeof val === "undefined" || val === null || val === "undefined"
          ? ""
          : val;
      return val;
    }
    ...
    function getValues(canTransform = true) {
      ...
      for (const key of keys) {
        ...
        if (typeof val !== "undefined") lodash_set(obj, key, val);
      }
    ...
    function updateDefaultValues(values, replaceInitial = false) {
      if (!values) return;
      if (replaceInitial) options.initialValues = values;
      ...
      for (const [key, val] of Object.entries(_elements)) {
        ...
        if (Array.isArray(value) && val.type !== "select-multiple") {
          console.warn(
            `Element ${key} default value is an Array but is not of type select-multiple. Only the first value will be used.`
          );
          value = value[0] || "";
        }
    ...
    function getNativeValidators(name) {
      const conf = _elements[name];
      if (!conf || !(conf.el instanceof HTMLElement)) return {};
      const el = conf.el;
    ...
    async function onChange(eventOrName, value) {
      ...
      if (isVirtual) setValue(name, value);
      ...
      if (options.validateChange) await validate(name);
    }
    /**
     * Handles form reset.
     *
    ...
    async function reset(values, replaceDefaults = false) {
      ...
      if (options.validateInit) await validate();
    }
    function submit(e) {
      onSubmit(e);
    }
    function field(nameOrElement, value) {
      ...
      const isElement =
        typeof window !== "undefined" && newEl instanceof Element;
    ...
    function bind(...collection) {
      for (const el of collection) {
        ...
        const key = el.name;
        let defaultValue = options.defaultGetter(
          key,
          el,
        ...
        if (Array.isArray(defaultValue) && el.type !== "select-multiple") {
          console.warn(
            `Element ${el.name} default value is an Array but is not of type select-multiple. Only the first value will be used.`
          );
        ...
        if (el.type === "radio") {
          ...
          if (conf.el.includes(el)) continue;
          conf.el.push(el);
          conf.type = el.type;
        ...
        if (typeof defaultValue !== "undefined") setValue(key, defaultValue);
    ...
    function unbind(...collection) {
      for (const el of collection) {
        ...
        if (Array.isArray(conf.el)) {
          ...
          if (!len) delete _elements[el.name];
        } else {
    ...
    async function initForm(form) {
      ...
      if (options.subscribe) subscribe(options.subscribe);
      if (options.validateInit) await validate(options.initialValues);
      else
        setTimeout(() => {
          updateFormState({});
        });
    ...
    function register(f) {
      ...
      _unsubscribeMutationObserver = createMutationObserver(
        f,
        (el, type) => {
          if (_formState.submitted) return;
          if (type === "remove") unbind(el);
          else bind(el);
        },
   ...
   */
  const defaultGetter = (name, el, values) => {
  ...
  function useKensho(options) {
    ...
    const { set, ...store } = createStore({});
    ctrl.subscribe(set);
    const _store = store;
    _store.subscribe = (...args) => ctrl.subscribe(...args);
    return {
      ...ctrl,
      store: _store,
    };
  ...
  exports.HTML5_INPUT_TYPES = HTML5_INPUT_TYPES;
  exports.HTML5_PLACEHOLDER_TYPES = HTML5_PLACEHOLDER_TYPES;
  exports.MAX_DEPTH = MAX_DEPTH;
  exports.VALID_MUTATION_NODES = VALID_MUTATION_NODES;
  exports["default"] = useKensho;
  exports.helpers = helpers;
  exports.useKensho = useKensho;
