...
const Select: FC<InputProps> = (props) => {
  ...
  let selectClass = 'block w-full px-4 py-2.5 mt-2 text-gray-700 bg-white border border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 focus:ring-opacity-40  focus:outline-none focus:ring shadow-sm rounded-sm';

  if (!rest.multiple)
    selectClass = selectClass + ' single';

  return (
    <div>
      {!label ? null :
        <label className="text-gray-700 text-sm block text-sm font-medium" htmlFor={rest.name}>
          {capitalize(label as string)}
        </label>
      }
      <select {...rest} data-unbound={unbound} data-datatype={datatype} className={selectClass} />
      <style jsx>{`
          select.single {
