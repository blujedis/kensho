...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["@kensho/adapter-react"] = {}, global.React));
  ...
  function useKensho(options) {
      ...
      const ctrl = react.useMemo(() => createController({ ...options, adapter: 'react', defaultGetter }), [options]);
      const register = ctrl.register;
      ctrl.register = react.useCallback((f) => {
          if (!f) {
              return { destroy: () => { } };
          }
          return register(f);
      }, []);
      ctrl.subscribe = react.useCallback(ctrl.subscribe, []);
      ctrl.getValue = react.useCallback(ctrl.getValue, []);
      ctrl.getValues = react.useCallback(ctrl.getValues, []);
      ctrl.setValue = react.useCallback(ctrl.setValue, []);
      ctrl.setValues = react.useCallback(ctrl.setValues, []);
      ctrl.field = react.useCallback(ctrl.field, []);
      ctrl.submit = react.useCallback(ctrl.submit, []);
      ctrl.reset = react.useCallback(ctrl.reset, []);
      ctrl.validate = react.useCallback(ctrl.validate, []);
