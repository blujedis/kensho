...
function useKensho(options) {
    ...
    const ctrl = useMemo(() => createController({ ...options, adapter: 'react', defaultGetter }), [options]);
    const register = ctrl.register;
    ctrl.register = useCallback((f) => {
        if (!f) {
            return { destroy: () => { } };
        }
        return register(f);
...
export { DEFAULT_FORM_STATE, HTML5_INPUT_TYPES, HTML5_PLACEHOLDER_TYPES, MAX_DEPTH, VALID_MUTATION_NODES, useKensho as default, ensureArray, flattenKeys, flattenMap, getFirstDefined, lodash_get as getProperty, lodash_has as hasProperty, mergeObject, lodash_set as setProperty, unflattenMap };
