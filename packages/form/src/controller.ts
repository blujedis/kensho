...
>(options: FormOptions<T & { [key: string]: unknown }, F>) {
	 ...
	 */
	async function validate<U extends Record<string, unknown>>(
		values: U
	): Promise<ErrorState<Data, F>>;
	...
	async function validate(
		names?: ElementKey | ElementKey[]
	): Promise<ErrorState<Data, F>>;
	async function validate<U extends Record<string, unknown>>(
		namesOrValues?: ElementKey | ElementKey[] | U
	) {
		...
		if (!invalid) {
			...
			_errorState = (names as ElementKey[]).reduce((a, c) => {
				...
				else if (!options.flattenErrors && errors) setProperty(a, c, errors);
				return a;
			}, {} as any);
		}
	...
	function getValue(name: ElementKey, cast = options.onCoerce): FormElementValue {
		...
		const handlers = TYPE_MAP[
			conf.type as keyof typeof TYPE_MAP
		] as ElementGetterSetter;
	...
	function getValues(canTransform = true): Required<Data> {
		...
		for (const key of keys) {
			...
			if (typeof val !== 'undefined') setProperty(obj, key, val);
		}
	...
	function updateDefaultValues(
		...
		if (!values) return;
		if (replaceInitial) options.initialValues = values as Data;
	 ...
	 */
	async function reset<U extends Record<string, unknown> = Data>(
		values?: U,
	...
	function bind(...collection: FormElement[]) {
		for (const el of collection) {
			...
			if (el.type === 'radio') {
				...
				conf.type = el.type;
				conf.defaultValue =
					defaultValue || ((el as HTMLInputElement).checked && el.value) || '';
	...
	async function initForm(form: HTMLFormElement) {
		...
		if (options.validateInit) await validate(options.initialValues as Data);
		else
			setTimeout(() => {
				updateFormState({});
			});
	...
	function register(f: HTMLFormElement | null) {
		...
		_unsubscribeMutationObserver = createMutationObserver(
			...
			(el, type) => {
				if (_formState.submitted) return;
				if (type === 'remove') unbind(el as FormElement);
				else bind(el as FormElement);
			},
