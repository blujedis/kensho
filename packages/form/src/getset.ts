...
type OptionType = (HTMLOptGroupElement | HTMLOptionElement) & {
	selected?: boolean;
};
...
export function setSelectValue(conf: FormElementConfig, value: FormElementValue) {
	...
	if (conf.virtual || !el) {
		conf.virtualValue =
			conf.type === 'select-multiple' ? ensureArray(value as string[]) : value;
	} else {
		const _values = ensureArray(value as string[]).filter(
