...
export declare type FormElementValue = string | number | readonly string[] | undefined;
...
export declare type DataTypeConfig = DataType | `[${Exclude<DataTypeBase, 'none'>}]|${string}`;
...
export declare type FormElementGetter = (conf: FormElementConfig) => FormElementValue;
export declare type FormElementSetter = (conf: FormElementConfig, value: FormElementValue) => void;
...
export interface SubscribeState<T extends Record<string, unknown>, F extends boolean = false> extends FormState {
    fields: FieldStateMap<T & {
        [key: string]: any;
    }>;
    errors: ErrorState<T, F>;
}
export declare type TransformHandler<T extends Record<string, unknown>> = (values: T) => Record<string, unknown>;
export declare type CoerceHandler = (key: string, value: FormElementValue, options: {
    dataType: any;
    dataTypeOptions: any[];
}) => FormElementValue;
export declare type AdapterType = 'react' | 'angular' | 'vue' | 'svelte' | 'none';
...
export declare type FormAdapterOptions<T extends Record<string, unknown>, F extends boolean = false> = Omit<FormOptions<T, F>, 'adapter' | 'subscribe' | 'defaultGetter'>;
