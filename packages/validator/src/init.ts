...
function createParsers(
	handlers: ValidatorDefineHandler[],
	...
	function parse(value: any, data?: Record<string, unknown>) {
		...
		if (Array.isArray(value)) throw new Error(`Parse value cannot be an array.`);
		handlers = ensureArray(handlers);
		if (!handlers.length) return [];
		for (const handler of handlers) {
			if (options.depth > 0 && errors.length >= options.depth) return errors;
			const message = handler(value, data);
			if (typeof message === 'string' && message.length) errors.push(message);
		}
		return errors;
	}
...
function initMethods<
	T extends Record<string, CreateValidatorHandler>,
	O extends ValidatorOptions
>(validators: T, options: O) {
...
>(validators = {} as T, options = { depth: 0, flatten: false } as ValidatorOptions) {
	...
	function object<C extends ApiConfig<T>>(config: C) {
		...
		>(
			...
			values: V,
			..._args: any[]
		): A extends true ? Promise<ParsedErrors<V, F>> : ParsedErrors<V, F> {
			...
			function recurseConfig(conf: C, parent = '') {
				for (const [key, val] of Object.entries(conf)) {
					if (((val || {}) as any).__kensho__ !== true) {
						...
						const messages = (val as any).parse(
							dataVal,
							values
			...
			async function recurseConfigAsync(conf: C, parent = '') {
				for (const [key, val] of Object.entries(conf)) {
					if (((val || {}) as any).__kensho__ !== true) {
						...
						const dataVal = getProperty(values, path);
						const messages = (await (val as any).parseAsync(
							dataVal,
							values
						)) as unknown as string[];
						const k = path as keyof typeof errors;
			...
			if (async) return recurseConfigAsync(config) as any;
		 ...
		 * @param args any additional args, making types play nice.
		 */
		function parseAsync<V extends Record<string, unknown>>(
			values: V,
			...args: any[]
		) {
		 ...
		 * @param args any additional args, making types play nice.
		 */
		function parseFlat<V extends Record<string, unknown>>(
			values: V,
			...args: any[]
		) {
		 ...
		 * @param args any additional args, making types play nice.
		 */
		function parseFlatAsync<V extends Record<string, unknown>>(
			values: V,
			...args: any[]
		) {
	 ...
	 */
	function extend<E extends Record<string, Record<string, CreateValidatorHandler>>>(
		extend: E,
