...
function required(message = 'Field is required.') {
	return (value: any) => {
		...
		if (typeof value === 'boolean' && value === false) valid = false;
...
function includes(
	...
	message = `Value must include {params[0]}`
) {
	return (value: any) => {
		if (isFalsey(value)) return '';
...
function excludes(
	tokens: string | (string | number | boolean)[],
	message = `Value must exclude {params[0]}`
) {
	return (value: any) => {
		if (isFalsey(value)) return '';
		if (Array.isArray(tokens)) {
...
function url(exp = EXP_URL as string | RegExp, message = 'Value is not a url.') {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_URL : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function email(
	exp = EXP_EMAIL as string | RegExp,
	message = 'Value is not an email address.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_EMAIL : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function tel(
	exp = EXP_TEL as string | RegExp,
	message = 'Value must be a phone number.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_TEL : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function password(
	exp = EXP_PASSWORD_COMPLEXITY as string | RegExp,
	message = 'Requires one of each. Upper, lower, numeric and a special character.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_PASSWORD_COMPLEXITY : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function ssn(
	exp = EXP_SSN as string | RegExp,
	message = 'Value must be an SSN number.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_SSN : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function ip(
	exp = EXP_IP_ADDR as string | RegExp,
	message = 'Value must be an ip address.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_IP_ADDR : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function mac(
	exp = EXP_MAC_ADDR as string | RegExp,
	message = 'Value must be a mac address.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_MAC_ADDR : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function zip(
	exp = EXP_ZIP as string | RegExp,
	message = 'Value must be a zip code.'
) {
	return (value: any) =>
		pattern(
			typeof exp === 'string' ? EXP_ZIP : exp,
			typeof exp === 'string' ? exp : message
		)(value);
}
...
function equals(compare: any, message = 'Values must be equal.') {
	return (value: any) => {
		...
		if (Array.isArray(value)) {
			if (!Array.isArray(compare) || compare.length !== value.length) return message;
			const cloneVal = [...value].sort();
			const cloneCompare = [...compare].sort();
			for (let i = 0, el = cloneVal[i]; i < cloneVal.length; i++) {
				if (el !== cloneCompare[i]) return message;
			}
			return '';
		} else {
...
function length(len: number, message = 'Value must be length of {params[0]}') {
	return (value: any) => {
		if (isFalsey(value)) return '';
		let valid = !isFalsey(value);
		if (Array.isArray(value)) valid = value.length === len;
		else if (typeof value === 'string') valid = value.length === len;
		return valid ? '' : message;
	};
}
...
function filetype(
	...
	message = 'File input contains invalid mime type.'
) {
...
function filemax(max: number, message = 'File input is too large.') {
	return (value: File | File[]) => {
		value = ensureArray(value);
		if (isFalsey(value) || !(value[0] instanceof File)) return '';
